...
# Security-sensitive settings that fall back to a built-in value. The app
# still starts (behaviour is unchanged), but a deployment running on any of
# these is using a key that is public in the source tree.
INSECURE_DEFAULTS = {
    'FLASK_SECRET_KEY': 'default-dev-key',
    'SECRET_KEY': 'default-dev-key',
    'MFA_ENCRYPTION_KEY': 'x_uJvYq82bW40P-ZfJ0o1W7aM8R3_59gXhT3iYf1cHg=',
}


def warn_on_insecure_defaults():
    """Log a warning for every secret still using its in-source fallback."""
    for name, fallback in INSECURE_DEFAULTS.items():
        if getattr(Config, name, None) == fallback:
            logging.warning(
                '[config] %s is using the built-in default. Set it in .env '
                'and rotate the old value.', name)
    if Config.JWT_SECRET_KEY == Config.SECRET_KEY == INSECURE_DEFAULTS['SECRET_KEY']:
        logging.warning('[config] JWT_SECRET_KEY is inheriting the default SECRET_KEY.')
