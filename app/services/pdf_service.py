...
# Resolved from the package root: this module now lives in app/services/,
# while the TTF files stay at the project root and the PDF template ships
# inside app/static/.
...
class PDFGenerator:
    ...
    def generate_property_report(self, properties: List[Dict], total_properties: int, avg_price: float, 
        ...
        # Anchored to the project root, not the process CWD. Flask resolves a
        # relative send_file() path against app.root_path, which is the package
        # directory -- a bare filename here would be looked for in app/.
        final_output_path = os.path.join(
            PROJECT_ROOT, f"sardo_property_report_{today_date}_{client_name_upper}.pdf")
        ...
        template_path = os.path.join(PACKAGE_DIR, "static", "pdfs", "Property_Portfolio_Template.pdf")
        ...
        if os.path.exists(template_path):
            try:
                ...
                logging.error(f"Error reading template PDF: {e}")
        ...
        try:
            ...
            logging.error(f"Error loading dynamic pages: {e}")
    ...
    def _create_property_overview_page(self, property_data: Dict, index: int) -> List:
        ...
        if image_filename:
            try:
                ...
                logging.error(f"Error loading hero image {image_filename}: {str(e)}")
        ...
        try:
            ...
            for col in ('image_filename_2', 'image_filename_3'):
                ...
                if val and val != "N/A":
                    try:
                        ...
                        logging.error(f"Error loading S3 sub image {val}: {s3_err}")
            ...
            if len(sub_images) < 2:
                ...
                if len(candidate_urls) < 2 and property_data.get('property_url'):
                    ...
                    try:
                        ...
                        logging.error(f"Error crawling live images: {crawl_err}")
                ...
                for url in candidate_urls:
                    ...
                    if url.startswith(('http://', 'https://')):
                        try:
                            ...
                            logging.error(f"Error downloading direct image {url}: {download_err}")
                    else:
                        s3_key = url
            ...
            logging.error(f"Error loading supporting images: {e}")
        ...
        if first_seen_str:
            try:
                ...
                logging.error(f"DOM Parse Error: {e} | value: {first_seen_str}")
        ...
        if map_filename and map_filename != "N/A":
            try:
                ...
                logging.error(f"Error loading map image {map_filename}: {e}")
